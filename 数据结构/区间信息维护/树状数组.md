...
class BIT {
  public:
    BIT() : sum1(MAX), sum2(MAX) {}
    //利用差分建树(如果序列有初始值)
    BIT(vector<gg>& v) : sum1(MAX), sum2(MAX) {
        for (gg i = 1; i <= ni; ++i) {
            update(i, v[i] - v[i - 1]);
        }
    }
...
class BIT {
  public:
    BIT() : sum(MAX, vector<gg>(MAX)) {}
    //将A[x][y]的值增加v
    // ni，mi分别为输入矩阵的长宽
    void update(gg x, gg y, gg v) {
        for (gg i = x; i <= ni; i += lowbit(i)) {
            for (gg j = y; j <= mi; j += lowbit(j)) {
                sum[i][j] += v;
            }
        }
    }
...
class BIT {
  public:
    BIT() : sum(MAX, vector<gg>(MAX)) {}
    //利用差分建树(如果序列有初始值)
    BIT(vector<vector<gg>>& v) : sum(MAX, vector<gg>(MAX)) {
        for (gg i = 1; i <= ni; ++i) {
            for (gg j = 1; j <= mi; ++j) {
                update(i, j,
...
class BIT {
  public:
    BIT() :
        sum1(MAX, vector<gg>(MAX)), sum2(MAX, vector<gg>(MAX)),
        sum3(MAX, vector<gg>(MAX)), sum4(MAX, vector<gg>(MAX)) {}
