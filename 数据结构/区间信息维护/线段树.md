...
class SegmentTree {
  public:
    SegmentTree(gg len) : n(len), vi(len), sumTree((len + 5) * 4) {
        for (gg i = 1; i <= n; ++i) {
            cin >> vi[i];  //读取原始输入序列
        }
    ...
    void realGetSum(gg left, gg right) { update(1, 1, n, left, right); }
...
class SegmentTree {
  public:
    SegmentTree(gg len) : n(len), vi(len), sumTree((len + 5) * 4), lazy((len + 5) * 4) {
        for (gg i = 1; i <= n; ++i) {
            cin >> vi[i];  //读取原始输入序列
        }
        init(1, 1, n);  //根据输入序列vi建立线段树
...
class SegmentTree {
  public:
    SegmentTree(gg len, gg p = (1ull << 63) - 1) :
        n(len), sumTree((len + 5) * 4), mul((len + 5) * 4, 1), add((len + 5) * 4), vi(len), mod(p) {
        for (gg i = 1; i <= n; ++i) {
            cin >> vi[i];  //读取原始输入序列
        }
        init(1, 1, n);
