...
class SegmentTree {
  public:
    SegmentTree() : vi(MAX), sumTree(MAX * 4) {
        for (gg i = 1; i <= ni; ++i) {
            cin >> vi[i];  //读取原始输入序列
        }
        init(1, 1, ni);  //根据输入序列vi建立线段树
    }
    //将vi[p]增加v
    void realUpdate(gg p, gg v) { update(1, 1, ni, p, v); }
    //查询vi的[left,right]区间的和
    void realGetSum(gg left, gg right) { update(1, 1, ni, left, right); }
...
class SegmentTree {
  public:
    SegmentTree() : vi(MAX), sumTree(MAX * 4), lazy(MAX * 4) {
        for (gg i = 1; i <= ni; ++i) {
            cin >> vi[i];  //读取原始输入序列
        }
        init(1, 1, ni);  //根据输入序列vi建立线段树
    ...
    void realUpdate(gg left, gg right, gg v) {
        update(1, 1, ni, left, right, v);
    }
    //查询vi的[left,right]区间的和
    gg realGetSum(gg left, gg right) { return getSum(1, 1, ni, left, right); }
...
class SegmentTree {
  public:
    SegmentTree(gg p = (1ull << 63) - 1) :
        sumTree(MAX * 4), mul(MAX * 4, 1), add(MAX * 4), vi(MAX), mod(p) {
        for (gg i = 1; i <= ni; ++i) {
            cin >> vi[i];  //读取原始输入序列
        }
        init(1, 1, ni);
